...
class DataIngestion:
    ...
    def unzip_and_clean(self):
        with ZipFile(file=self.config.local_data_file,mode="r") as zf:
            ...
            updated_list_of_file=self.get_updated_list_of_files(list_of_file) #here we are calling another method defined in the same class
            for f in tqdm(updated_list_of_file):
                self.preprocess(zf, f, self.config.unzip_dir) #here we are calling another method of the same class
